...
class TestLoginPage:
    ...
    def test_login_accros_button_enter_account(self, driver):
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located((TestLocators.BUTTON_ENTER_IN_ACCOUNT)))
        ...
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located((TestLocators.BUTTON_COLLECT_ORDER)))
        button_text = driver.find_element(*TestLocators.BUTTON_COLLECT_ORDER).text
        assert button_text == 'Оформить заказ'

    # вход через кнопку «Личный кабинет»
    def test_login_accros_account(self, driver):
        WebDriverWait(driver, 10).until(
    ...
    def test_login_accros_registration_page(self, driver):
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located((TestLocators.BUTTON_ACCOUNT)))
        driver.find_element(*TestLocators.BUTTON_ACCOUNT).click()
    ...
    def test_login_accros_recovery_page(self, driver):
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located((TestLocators.BUTTON_ENTER_IN_ACCOUNT)))
        driver.find_element(*TestLocators.BUTTON_ENTER_IN_ACCOUNT).click()
